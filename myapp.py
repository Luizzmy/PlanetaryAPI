...
app.config['JWT_SECRET_KEY']='ElSecretoDeAmor'  #Should be changed in a real project
app.config['MAIL_SERVER']='smtp.mailtrap.io'
app.config['MAIL_PORT'] = 2525
app.config['MAIL_USERNAME'] = 'bec2db0ef5ad11'
app.config['MAIL_PASSWORD'] = '4bf90f9fa13119'
...
def db_seed():
    ...
    test_user=User(first_name='Luis',
...
def super_simple():
    return jsonify(message='Hello from the Planetary API. boo '), 200
...
@app.route('/action/<string:x>/<string:y>')
def action(x:str,y:str):
    return jsonify(result=x+y)


@app.route('/planets', methods=['GET'])
def planets():
    planets_list=Planet.query.all()    #Command from SQLAlchemy
...
def login():
    if request.is_json:
        ...
        if test:
            ...
            return jsonify(message='Login succeeded', access_token=access_token), 401
        else:
...
def update_planet():
    ...
    if planet:
        ...
        return jsonify(mesage='That planet does not exist'), 404
